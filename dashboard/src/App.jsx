...
// ---- Overview page ----
function OverviewPage({ farms, selectedFarmId, setSelectedFarmId, selectedMetric, setSelectedMetric, handlePolygonDrawn, handleDeleteFarm, setActiveNav }) {
          ...
          {selectedFarm ? (
            <>
              <div className="flex items-center justify-between mb-4 px-1">
                <h2 className="text-base font-bold text-gray-900">{selectedFarm.name}</h2>
                <span className="text-sm font-medium px-3 py-1 rounded-full bg-violet-100 text-violet-600">{selectedFarm.group}</span>
              </div>
              <div className="flex-1 overflow-y-auto min-h-0">
...
// ---- Map page ----
function MapPage({ farms, selectedFarmId, setSelectedFarmId, handlePolygonDrawn }) {
  return (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-[30px]">
      <div className="lg:col-span-1 bg-white rounded-2xl shadow-sm p-3">
        ...
        <PortfolioPanel
          ...
          onSelectFarm={(id) => { setSelectedFarmId(id); setActiveNav("farm"); }}
  ...
  if (!farm) {
    return (
      <div className="flex flex-col items-center justify-center h-96 text-gray-300">
        ...
        <p className="text-lg font-medium text-gray-400">Select a farm from the Portfolio to view details</p>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {farm.metricsData.realDataLoaded && (
              <span className="text-sm font-bold px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-600">LIVE DATA</span>
            )}
...
export default function App() {
  ...
  // Navigate to farm view when selecting a farm from overview
  const handleFarmSelectAndNav = useCallback((id) => {
  ...
  return (
    <div className="min-h-screen bg-[#f1f0f9] flex">
      ...
      {/* ---- Sidebar ---- */}
      <aside className="w-60 bg-white flex flex-col flex-shrink-0 border-r border-gray-100 min-h-screen">
        {/* Logo */}
        <div className="px-6 pt-6 pb-7 flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-base font-extrabold text-white">
            F
          </div>
          <span className="text-lg font-bold text-gray-900 tracking-tight">FarmRisk</span>
        </div>
        ...
        <div className="px-5 mb-6">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-violet-500 to-purple-600 text-white text-sm font-bold rounded-xl px-4 py-3 shadow-md shadow-violet-200 hover:shadow-lg hover:shadow-violet-300 transition-all"
          >
            Add farm
        ...
        <nav className="flex-1 px-4 space-y-1.5">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                <svg className={`w-5 h-5 ${isActive ? "text-violet-500" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        <header className="bg-white/70 backdrop-blur-md border-b border-gray-100 sticky top-0 z-40">
          <div className="px-7 py-4 flex items-center justify-between">
            <h2 className="text-lg font-bold text-gray-900 capitalize">{activeNav === "farm" && selectedFarm ? selectedFarm.name : activeNav}</h2>
            <div className="flex items-center gap-4">
              {farms.length > 0 && (
                <span className="text-sm font-medium px-4 py-2 rounded-full bg-violet-100 text-violet-600">
                  {farms.length} farm{farms.length !== 1 ? "s" : ""} in portfolio
                </span>
              )}
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-violet-400 to-purple-500 flex items-center justify-center text-sm font-bold text-white">
                U
              </div>
            </div>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-7">
          {activeNav === "overview" && (
            <OverviewPage
              farms={farms}
              selectedFarmId={selectedFarmId}
              setSelectedFarmId={handleFarmSelectAndNav}
